...
describe BlocksController do
  ...
  describe "GET index" do
    it "assigns all blocks as @blocks" do
      Block.stub(:find).with(:all).and_return([mock_block])
      get :index
      assigns[:blocks].should == [mock_block]
    end
  end

  describe "GET show" do
    it "assigns the requested block as @block" do
      Block.stub(:find).with("37").and_return(mock_block)
      get :show, :id => "37"
      assigns[:block].should equal(mock_block)
    end
  end

  describe "GET new" do
    it "assigns a new block as @block" do
      Block.stub(:new).and_return(mock_block)
      get :new
      assigns[:block].should equal(mock_block)
    end
  end
  ...
  describe "POST create" do
    ...
    describe "with valid params" do
      it "assigns a newly created block as @block" do
        Block.stub(:new).with({'these' => 'params'}).and_return(mock_block(:save => true))
        post :create, :block => {:these => 'params'}
        assigns[:block].should equal(mock_block)
      end

      it "redirects to the created block" do
        Block.stub(:new).and_return(mock_block(:save => true))
        post :create, :block => {}
    ...
    describe "with invalid params" do
      it "assigns a newly created but unsaved block as @block" do
        Block.stub(:new).with({'these' => 'params'}).and_return(mock_block(:save => false))
        post :create, :block => {:these => 'params'}
        assigns[:block].should equal(mock_block)
      end

      it "re-renders the 'new' template" do
        Block.stub(:new).and_return(mock_block(:save => false))
        post :create, :block => {}
  ...
  describe "PUT update" do
    ...
    describe "with valid params" do
      it "updates the requested block" do
        Block.should_receive(:find).with("37").and_return(mock_block)
        mock_block.should_receive(:update_attributes).with({'these' => 'params'})
        put :update, :id => "37", :block => {:these => 'params'}
      end

      it "assigns the requested block as @block" do
        Block.stub(:find).and_return(mock_block(:update_attributes => true))
        put :update, :id => "1"
        assigns[:block].should equal(mock_block)
      end
    ...
    describe "with invalid params" do
      ...
      it "assigns the block as @block" do
        Block.stub(:find).and_return(mock_block(:update_attributes => false))
        put :update, :id => "1"
        assigns[:block].should equal(mock_block)
      end
  ...
  describe "DELETE destroy" do
    ...
    it "redirects to the blocks list" do
      Block.stub(:find).and_return(mock_block(:destroy => true))
      delete :destroy, :id => "1"
